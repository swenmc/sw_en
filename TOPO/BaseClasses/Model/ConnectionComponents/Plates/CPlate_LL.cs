...
namespace BaseClasses
{
    ...
    public class CConCom_Plate_LL : CPlate
    {
        ...
        public override void UpdatePlateData(CScrewArrangement screwArrangement)
        {
            ...
            {
                ((CScrewArrangement_LL)screwArrangement).Calc_HolesCentersCoord2D(Ft, m_fbX1, m_fbX2, m_fhY, m_flZ);
                arrConnectorControlPoints3D = new Point3D[screwArrangement.IHolesNumber];
                Calc_HolesControlPointsCoord3D((CScrewArrangement_LL)screwArrangement);
            }
        ...
        void GenerateConnectors(CScrewArrangement screwArrangement)
        {
            if (screwArrangement.IHolesNumber > 0)
            {
                ...
                for (int i = 0; i < screwArrangement.IHolesNumber; i++)
                {
                    ...
                    {
                        Point3D controlpoint = new Point3D(arrConnectorControlPoints3D[i].X, arrConnectorControlPoints3D[i].Y, arrConnectorControlPoints3D[i].Z);
                        screwArrangement.Screws[i] = new CScrew(screwArrangement.referenceScrew, controlpoint, 0, 0, 0);
                    }
                    else if (i < screwArrangement.IHolesNumber * 3 / 4) // Front Right
                    {
                        Point3D controlpoint = new Point3D(arrConnectorControlPoints3D[i].X, arrConnectorControlPoints3D[i].Y, arrConnectorControlPoints3D[i].Z);
                        screwArrangement.Screws[i] = new CScrew(screwArrangement.referenceScrew, controlpoint, 0, 180, 0);
                    }
                    else // Right
                    {
                        Point3D controlpoint = new Point3D(arrConnectorControlPoints3D[i].X, arrConnectorControlPoints3D[i].Y, arrConnectorControlPoints3D[i].Z);
