...
namespace BaseClasses
{
    public class CConnectionJoint_TD01 : CConnectionJointTypes
    {
        ...
        public CConnectionJoint_TD01(EJointType jointType_temp, CNode Node_temp, CMember MainFrameColumn_temp)
        {
            ...
            Point3D ControlPoint_P1 = new Point3D(fAlignment_x, - 0.5f * fb_plate - m_ft + flocaleccentricity_y, -0.5f * fh_plate + flocaleccentricity_z);
        ...
        {
            return new CConnectionJoint_TD01(JointType, m_Node, m_MainMember);
        }
        ...
        public override void UpdateJoint()
        {
            ...
            m_arrPlates[0].ControlPoint = new Point3D(0, -0.5f * m_arrPlates[0].Width_bx - m_arrPlates[0].Ft + flocaleccentricity_y, -0.5f * m_arrPlates[0].Height_hy + flocaleccentricity_z);
            Vector3D RotationVector = new Vector3D(90, 0, 90);

            if (m_Node.ID != m_MainMember.NodeStart.ID) // If true - joint at start node, if false joint at end node (se we need to rotate joint about z-axis 180 deg)
            {
                // Rotate and move joint defined in the start point [0,0,0] to the end point
                m_arrPlates[0].ControlPoint = new Point3D(m_MainMember.FLength, +0.5f * m_arrPlates[0].Width_bx - m_arrPlates[0].Ft + flocaleccentricity_y, -0.5f * m_arrPlates[0].Height_hy + flocaleccentricity_z);
                RotationVector = new Vector3D(90, 0, 180 + 90);
            }
