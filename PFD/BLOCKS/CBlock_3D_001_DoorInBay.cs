    ...
    public class CBlock_3D_001_DoorInBay : CBlock
    {
            ...
            bool bIsLastBayInFrontorBackSide = false)
        {
            ...
            if (prop.sDoorType == "Personnel Door")
            {
                // Personnel door
                // One cross-section
                m_arrCrSc[(EMemberType_FS_Position)1] = new CCrSc_3_10075_BOX(0, 0.1f, 0.1f, 0.00075f, Colors.Red); // Door frame
                m_arrCrSc[(EMemberType_FS_Position)1].Name_short = "10075";
                m_arrCrSc[(EMemberType_FS_Position)1].m_Mat = m_arrMat[0];
                m_arrCrSc[(EMemberType_FS_Position)1].ID = (int)EMemberType_FS_Position.DoorFrame;
                ...
                crscColumn = m_arrCrSc[(EMemberType_FS_Position)1];
                crscLintel = m_arrCrSc[(EMemberType_FS_Position)1];
            }
            else // if (prop.sDoorType == "Roller Door")
            {
                ...
                // Trimmer
                m_arrCrSc[(EMemberType_FS_Position)1] = new CCrSc_3_270XX_C_BACK_TO_BACK(0, 0.27f, 0.14f, 0.02f, 0.00115f, Colors.Beige); // Door trimmer
                m_arrCrSc[(EMemberType_FS_Position)1].Name_short = "270115btb";
                m_arrCrSc[(EMemberType_FS_Position)1].m_Mat = m_arrMat[0];
                m_arrCrSc[(EMemberType_FS_Position)1].ID = (int)EMemberType_FS_Position.DoorTrimmer;
                ...
                // Lintel
                m_arrCrSc[(EMemberType_FS_Position)2] = new CCrSc_3_270XX_C(0, 0.27f, 0.07f, 0.00095f, Colors.Chocolate); // Door lintel
                m_arrCrSc[(EMemberType_FS_Position)2].Name_short = "27095";
                m_arrCrSc[(EMemberType_FS_Position)2].m_Mat = m_arrMat[0];
                m_arrCrSc[(EMemberType_FS_Position)2].ID = (int)EMemberType_FS_Position.DoorLintel;

                eTypeLintel = EMemberType_FS.eDL;
                eTypePositionLintel = EMemberType_FS_Position.DoorLintel;
                crscLintel = m_arrCrSc[(EMemberType_FS_Position)2];
            }
                    ...
                    if (i == 1 || bDoorToCloseToLeftColumn) // If just right sequence of girts is generated switch alignment and eccentricity (???) need testing;
                    {
                        if (!bIsLastBayInFrontorBackSide) // Change alignment (different columns on bay sides)
                        {
                            ...
                            if(prop.sBuildingSide == "Left" || prop.sBuildingSide == "Right")
                                fGirtEndTemp = ReferenceGirt.FAlignment_End; // Ak su girt len na pravej strane od dveri, nastavime koncove odsadenie podla povodneho koncoveho odsadenia referencneho girt
