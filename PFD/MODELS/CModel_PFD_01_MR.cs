...
namespace PFD
{
    ...
    {
        private int m_LeftColumnGirtNo;
        private int m_RightColumnGirtNo;
        ...
        //--------------------------------------------------------------------------------------------------------------------
        //--------------------------------------------------------------------------------------------------------------------
        public int LeftColumnGirtNo
        {
            get
            {
                return m_LeftColumnGirtNo;
            }

            set
            {
                m_LeftColumnGirtNo = value;
            }
        }

        public int RightColumnGirtNo
        {
            get
            {
                return m_RightColumnGirtNo;
            }

            set
            {
                m_RightColumnGirtNo = value;
            }
        }
        ...
        )
        {
            ...
            fRoofPitch_rad = (float)Math.Atan((fH2_frame_centerline - fH1_frame_centerline) / fW_frame_centerline);
            ...
            */
            ...
            // Monopitch !!!
            //float fNegativeSlopeDifference_Column = (float)m_arrCrSc[EMemberType_FS_Position.MainColumn].h * (float)Math.Tan(fRoofPitch_rad);
            //float fNegativeSlopeDifference_Rafter = Geom2D.GetRightTriangleAltitude((float)m_arrCrSc[EMemberType_FS_Position.MainRafter].h, fRoofPitch_rad); // Vyska v pravouhlom trojuholniku
            ...
            float falignment_column_Right = -(float)m_arrCrSc[EMemberType_FS_Position.MainRafter].z_min / (float)Math.Cos(fRoofPitch_rad) + (-(float)m_arrCrSc[EMemberType_FS_Position.MainColumn].z_min) * (float)Math.Tan(Math.Abs(fRoofPitch_rad));
            ...
            float feccentricityEavePurlinLeft_z = -falignment_column + (float)m_arrCrSc[EMemberType_FS_Position.MainRafter].h * (float)Math.Cos(Math.Abs(fRoofPitch_rad)) - (float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].z_max;
            float feccentricityEavePurlinRight_z = -falignment_column_Right + (float)m_arrCrSc[EMemberType_FS_Position.MainColumn].h * (float)Math.Tan(Math.Abs(fRoofPitch_rad)) + (float)m_arrCrSc[EMemberType_FS_Position.MainRafter].h / (float)Math.Cos(Math.Abs(fRoofPitch_rad)) - (float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].b * (float)Math.Tan(Math.Abs(fRoofPitch_rad)) - (float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].z_max;
            ...
            */
            ...
            if (bGenerateGirts)
            {
                ...
                RightColumnGirtNo = (int)((fH2_frame_centerline - fUpperGirtLimit - fBottomGirtPosition) / fDist_Girt) + 1;
                iGirtNoInOneFrame = LeftColumnGirtNo + RightColumnGirtNo;
            }

            componentListVM.SetColumnFlyBracingPosition_Items(Math.Min(LeftColumnGirtNo, RightColumnGirtNo));  //zakomentovane 20.12.2019 - nechapem naco to tu je
            ...
            float fRafterLength = MathF.Sqrt(MathF.Pow2(fH2_frame_centerline - fH1_frame_centerline) + MathF.Pow2(fW_frame_centerline));
            ...
            if (bGenerateFrontGirts)
            {
                if (fRoofPitch_rad > 0)
                {
                    iFrontIntermediateColumnNodesForGirtsOneRafterNo = GetNumberofIntermediateNodesInColumnsForOneFrame(false, iOneRafterFrontColumnNo, fH1_frame_centerline, fBottomGirtPosition, fDist_FrontColumns, fz_UpperLimitForFrontGirts);
                    ...
                    // Ascending (from left)
                    //iFrontGirtsNoInOneFrame = LeftColumnGirtNo;
                    iArrNumberOfGirtsPerFrontColumnFromLeft = new int[iFrontColumnNoInOneFrame + 1];
                    iArrNumberOfGirtsPerFrontColumnFromLeft[0] = LeftColumnGirtNo;
                    ...
                    {
                        int temp = GetNumberofIntermediateNodesInOneColumnForGirts(false, fH1_frame_centerline, fBottomGirtPosition, fDist_FrontColumns, fz_UpperLimitForFrontGirts, i);
                ...
                {
                    iFrontIntermediateColumnNodesForGirtsOneRafterNo = GetNumberofIntermediateNodesInColumnsForOneFrame(true, iOneRafterFrontColumnNo, fH2_frame_centerline, fBottomGirtPosition, fDist_FrontColumns, fz_UpperLimitForFrontGirts);
                    iFrontIntermediateColumnNodesForGirtsOneFrameNo = 1 * iFrontIntermediateColumnNodesForGirtsOneRafterNo;
                    ...
                    //iFrontGirtsNoInOneFrame = RightColumnGirtNo;
                    iArrNumberOfGirtsPerFrontColumnFromLeft = new int[iFrontColumnNoInOneFrame + 1];
                    iArrNumberOfGirtsPerFrontColumnFromLeft[0] = RightColumnGirtNo;

                    // Number of girts under one rafter at the frontside of building - middle girts are considered twice
                    for (int i = 0; i < iOneRafterFrontColumnNo; i++)
                    {
                        int temp = GetNumberofIntermediateNodesInOneColumnForGirts(true, fH2_frame_centerline, fBottomGirtPosition, fDist_FrontColumns, fz_UpperLimitForFrontGirts, i);
                        //iFrontGirtsNoInOneFrame += temp;
                        iArrNumberOfGirtsPerFrontColumnFromLeft[i + 1] = temp;
                        iArrNumberOfNodesPerFrontColumnFromLeft[i] = temp;
                    }
            ...
            componentListVM.SetFrontColumnFlyBracingPosition_Items(Math.Min(LeftColumnGirtNo, RightColumnGirtNo)); //zakomentovane 20.12.2019 - nechapem naco to tu je
            ...
            if (bGenerateBackGirts)
            {
                if (fRoofPitch_rad > 0)
                {
                    iBackIntermediateColumnNodesForGirtsOneRafterNo = GetNumberofIntermediateNodesInColumnsForOneFrame(false, iOneRafterBackColumnNo, fH1_frame_centerline, fBottomGirtPosition, fDist_BackColumns, fz_UpperLimitForBackGirts);
                    ...
                    // Ascending (from left)
                    //iBackGirtsNoInOneFrame = LeftColumnGirtNo;
                    iArrNumberOfGirtsPerBackColumnFromLeft = new int[iBackColumnNoInOneFrame + 1];
                    iArrNumberOfGirtsPerBackColumnFromLeft[0] = LeftColumnGirtNo;

                    // Number of girts under one rafter at the frontside of building - middle girts are considered twice
                    for (int i = 0; i < iOneRafterBackColumnNo; i++)
                    {
                        int temp = GetNumberofIntermediateNodesInOneColumnForGirts(false, fH1_frame_centerline, fBottomGirtPosition, fDist_BackColumns, fz_UpperLimitForBackGirts, i);
                ...
                {
                    iBackIntermediateColumnNodesForGirtsOneRafterNo = GetNumberofIntermediateNodesInColumnsForOneFrame(true, iOneRafterBackColumnNo, fH2_frame_centerline, fBottomGirtPosition, fDist_BackColumns, fz_UpperLimitForBackGirts);
                    iBackIntermediateColumnNodesForGirtsOneFrameNo = 1 * iBackIntermediateColumnNodesForGirtsOneRafterNo;
                    ...
                    // Descending (from right)
                    //iBackGirtsNoInOneFrame = RightColumnGirtNo;
                    iArrNumberOfGirtsPerBackColumnFromLeft = new int[iBackColumnNoInOneFrame + 1];
                    iArrNumberOfGirtsPerBackColumnFromLeft[0] = RightColumnGirtNo;

                    // Number of girts under one rafter at the frontside of building - middle girts are considered twice
                    for (int i = 0; i < iOneRafterBackColumnNo; i++)
                    {
                        int temp = GetNumberofIntermediateNodesInOneColumnForGirts(true, fH2_frame_centerline, fBottomGirtPosition, fDist_BackColumns, fz_UpperLimitForBackGirts, i);
                        //iBackGirtsNoInOneFrame += temp;
                        iArrNumberOfGirtsPerBackColumnFromLeft[i + 1] = temp;
                        iArrNumberOfNodesPerBackColumnFromLeft[i] = temp;
                    }
            ...
            componentListVM.SetBackColumnFlyBracingPosition_Items(Math.Min(LeftColumnGirtNo, RightColumnGirtNo)); //zakomentovane 20.12.2019 - nechapem naco to tu je
            ...
            {
                iNumberOfGBSideWallsNodesInOneBayOneSideLeft = iNumberOfTransverseSupports_Girts * (LeftColumnGirtNo + 1);
                iNumberOfGBSideWallsNodesInOneBayOneSideRight = iNumberOfTransverseSupports_Girts * (RightColumnGirtNo + 1);
                ...
                iNumberOfGBSideWallsMembersInOneBayOneSideLeft = iNumberOfTransverseSupports_Girts * LeftColumnGirtNo;
                iNumberOfGBSideWallsMembersInOneBayOneSideRight = iNumberOfTransverseSupports_Girts * RightColumnGirtNo;
            ...
            if (bGenerateGirtBracingFrontSide)
            {
                ...
                //iArrGB_FS_NumberOfNodesPerBay[0] = (Math.Min(LeftColumnGirtNo, RightColumnGirtNo) + 1) * iNumberOfTransverseSupports_FrontGirts; // Pridame o jeden rad uzlov viac - nachadzaju sa na edge rafter
                //iArrGB_FS_NumberOfNodesPerBayFirstNode[0] = (Math.Min(LeftColumnGirtNo, RightColumnGirtNo) + 1);
                //iArrGB_FS_NumberOfMembersPerBay[0] = Math.Min(LeftColumnGirtNo, RightColumnGirtNo) * iNumberOfTransverseSupports_FrontGirts;
                //
            ...
            if (bGenerateGirtBracingBackSide)
            {
                ...
                // First bay - pocet girts urcime podla poctu uzlov pre girts na edge / main column
                //iArrGB_BS_NumberOfNodesPerBay[0] = (Math.Min(LeftColumnGirtNo, RightColumnGirtNo) + 1) * iNumberOfTransverseSupports_BackGirts; // Pridame o jeden rad uzlov viac - nachadzaju sa na edge rafter
                //iArrGB_BS_NumberOfNodesPerBayFirstNode[0] = (Math.Min(LeftColumnGirtNo, RightColumnGirtNo) + 1);
                //iArrGB_BS_NumberOfMembersPerBay[0] = Math.Min(LeftColumnGirtNo, RightColumnGirtNo) * iNumberOfTransverseSupports_BackGirts;
                //
            ...
            if (bGenerateRoofCrossBracing)
            {
                foreach (CCrossBracingInfo cb in vm._crossBracingOptionsVM.CrossBracingList)
                {
                    if (cb.Roof)
                    {
                        ...
                        iNumberOfCrossBracingMembers_Roof_Total += cb.NumberOfCrossBracingMembers_BayRoof; // Celkovy pocet prutov cross bracing pre roof v celom modeli // Rozne podla vstupu v GUI a ine pre  roof a monopitch
                    }
                }
            }
            ...
            for (int i = 0; i < iFrameNo; i++)
            {
                ...
                m_arrNodes[i * iFrameNodesNo + 2] = new CNode(i * iFrameNodesNo + 3, fW_frame_centerline, GetBaysWidthUntilFrameIndex(i), fH2_frame_centerline, 0);
                ...
                m_arrNodes[i * iFrameNodesNo + 3] = new CNode(i * iFrameNodesNo + 4, fW_frame_centerline, GetBaysWidthUntilFrameIndex(i), 00000, 0);
            ...
            for (int i = 0; i < iFrameNo; i++)
            {
                ...
                // Main Column
                m_arrMembers[(i * iEavesPurlinNoInOneFrame) + i * (iFrameNodesNo - 1) + 2] = new CMember((i * iEavesPurlinNoInOneFrame) + i * (iFrameNodesNo - 1) + 3, m_arrNodes[i * iFrameNodesNo + 2], m_arrNodes[i * iFrameNodesNo + 3], m_arrCrSc[eColumnType_Position], eColumnType, eColumnType_Position, null, null, fMainColumnEnd_Right, fMainColumnStart, 0f, 0);
                ...
                if (i < (iFrameNo - 1))
                {
                    ...
                    m_arrMembers[(i * iEavesPurlinNoInOneFrame) + i * (iFrameNodesNo - 1) + iFrameMembersNo + 1] = new CMember((i * iEavesPurlinNoInOneFrame) + i * (iFrameNodesNo - 1) + iFrameMembersNo + 1 + 1, m_arrNodes[i * iFrameNodesNo + 2], m_arrNodes[(i + 1) * iFrameNodesNo + 2], m_arrCrSc[EMemberType_FS_Position.EdgePurlin], EMemberType_FS.eEP, EMemberType_FS_Position.EdgePurlin, eccEavePurlinRight, eccEavePurlinRight, fEavesPurlinStart, fEavesPurlinEnd, 0f, 0);
            ...
            if (bGenerateGirts)
            {
                for (int i = 0; i < iFrameNo; i++)
                {
                    for (int j = 0; j < LeftColumnGirtNo; j++)
                    {
                        //task 600
                    ...
                    for (int j = 0; j < RightColumnGirtNo; j++)
                    {
                        //task 600
                        //m_arrNodes[i_temp_numberofNodes + i * iGirtNoInOneFrame + LeftColumnGirtNo + j] = new CNode(i_temp_numberofNodes + i * iGirtNoInOneFrame + LeftColumnGirtNo + j + 1, fW_frame, i * fL1_frame, fBottomGirtPosition + j * fDist_Girt, 0);
                        m_arrNodes[i_temp_numberofNodes + i * iGirtNoInOneFrame + LeftColumnGirtNo + j] = new CNode(i_temp_numberofNodes + i * iGirtNoInOneFrame + LeftColumnGirtNo + j + 1, fW_frame_centerline, GetBaysWidthUntilFrameIndex(i), fBottomGirtPosition + j * fDist_Girt, 0);
                        RotateFrontOrBackFrameNodeAboutZ(m_arrNodes[i_temp_numberofNodes + i * iGirtNoInOneFrame + LeftColumnGirtNo + j]);
                    }
                }
            }
            ...
            if (bGenerateGirts)
            {
                for (int i = 0; i < (iFrameNo - 1); i++)
                {
                    ...
                    for (int j = 0; j < RightColumnGirtNo; j++)
                    {
                        m_arrMembers[i_temp_numberofMembers + i * iGirtNoInOneFrame + LeftColumnGirtNo + j] = new CMember(i_temp_numberofMembers + i * iGirtNoInOneFrame + LeftColumnGirtNo + j + 1, m_arrNodes[i_temp_numberofNodes + i * iGirtNoInOneFrame + LeftColumnGirtNo + j], m_arrNodes[i_temp_numberofNodes + (i + 1) * iGirtNoInOneFrame + LeftColumnGirtNo + j], m_arrCrSc[EMemberType_FS_Position.Girt], EMemberType_FS.eG, EMemberType_FS_Position.Girt, eccentricityGirtRight_XB, eccentricityGirtRight_XB, fGirtStart, fGirtEnd, fGirtsRotation, 0);
                        RotateFrontOrBackFrameNodeAboutZ(m_arrNodes[i_temp_numberofMembers + i * iGirtNoInOneFrame + LeftColumnGirtNo + j]);
                        CreateAndAssignRegularTransverseSupportGroupAndLTBsegmentGroup(m_arrMembers[i_temp_numberofMembers + i * iGirtNoInOneFrame + LeftColumnGirtNo + j], iNumberOfTransverseSupports_Girts);
                    }
                }
            }
            ...
            if (bGenerateFrontGirts)
            {
                AddFrontOrBackGirtsNodes(true, iOneRafterFrontColumnNo, iArrNumberOfNodesPerFrontColumnFromLeft, i_temp_numberofNodes, iFrontIntermediateColumnNodesForGirtsOneRafterNo, fH1_frame_centerline, fDist_FrontGirts, fDist_FrontColumns, 0);
            }
            ...
            if (bGenerateBackGirts)
            {
                AddFrontOrBackGirtsNodes(true, iOneRafterBackColumnNo, iArrNumberOfNodesPerBackColumnFromLeft, i_temp_numberofNodes, iBackIntermediateColumnNodesForGirtsOneRafterNo, fH1_frame_centerline, fDist_BackGirts, fDist_BackColumns, fL_tot_centerline);
            }
            ...
            if (bGenerateGirtBracingSideWalls)
            {
                for (int i = 0; i < (iFrameNo - 1); i++)
                {
                    ...
                    for (int j = 0; j < (LeftColumnGirtNo + 1); j++) // Left side
                    {
                        float zCoord = j < LeftColumnGirtNo ? (fBottomGirtPosition + j * fDist_Girt) : fH1_frame_centerline;
                    ...
                    for (int j = 0; j < (RightColumnGirtNo + 1); j++) // Right side
                    {
                        float zCoord = j < RightColumnGirtNo ? (fBottomGirtPosition + j * fDist_Girt) : fH2_frame_centerline;
                        ...
                        for (int k = 0; k < iNumberOfTransverseSupports_Girts; k++)
                        {
                            ...
                            m_arrNodes[i_temp_numberofNodes + i * iNumberOfGBSideWallsNodesInOneBay + iNumberOfGBSideWallsNodesInOneBayOneSideLeft + j * iNumberOfTransverseSupports_Girts + k] = new CNode(i_temp_numberofNodes + i * iNumberOfGBSideWallsNodesInOneBay + iNumberOfGBSideWallsNodesInOneBayOneSideLeft + j * iNumberOfTransverseSupports_Girts + k + 1, fW_frame_centerline, GetBaysWidthUntilFrameIndex(i) + (k + 1) * fIntermediateSupportSpacingGirts, zCoord, 0);
                        }
                    }
                }
            }
            ...
            if (bGenerateGirtBracingSideWalls)
            {
                for (int i = 0; i < (iFrameNo - 1); i++)
                {
                    for (int j = 0; j < LeftColumnGirtNo; j++) // Left side
                    {
                        ...
                        if (j == LeftColumnGirtNo - 1) // Last
                            fGBSideWallEnd_Current = (float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].z_min + feccentricityEavePurlinLeft_z - fCutOffOneSide;
                    ...
                    for (int j = 0; j < RightColumnGirtNo; j++) // Right side
                    {
                        bool bDeactivateMember = false;
                        if (bUseGBEverySecond && j % 2 == 1) bDeactivateMember = true;
                        ...
                        if (j == RightColumnGirtNo - 1) // Last
                            fGBSideWallEnd_Current = (float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].z_min + feccentricityEavePurlinRight_z - fCutOffOneSide;
                ...
                for (int i = 0; i < (iFrameNo - 1); i++)
                {
                    ...
                    for (int j = 0; j < (iOneRafterPurlinNo + 1 + iLastPurlin); j++) // Left side - eave purlin and purlins
                    {
                        ...
                        else
                        {
                            ...
                            { x_glob = fW_frame_centerline; z_glob = fH2_frame_centerline; } // Right edge of roof - monopitch
                        }
            ...
            if (bGeneratePurlinBracing)
            {
                for (int i = 0; i < (iFrameNo - 1); i++)
                {
                    for (int j = 0; j < iOneRafterPurlinNo + iLastPurlin; j++) // Left side
                    {
                        ...
                        {
                            //float a = (-(float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].y_max - eccentricityEavePurlin.MFy_local) / (float)Math.Cos(Math.Abs(fRoofPitch_rad));
                            ...
                            float g = (float)m_arrCrSc[EMemberType_FS_Position.Purlin].h * (float)Math.Tan(Math.Abs(fRoofPitch_rad));
                            float h = (float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].b / (float)Math.Cos(Math.Abs(fRoofPitch_rad));
                        ...
                        for (int k = 0; k < iNumberOfTransverseSupports_Purlins; k++)
                        {
                            ...
                            m_arrMembers[memberIndex] = new CMember(memberIndex + 1, m_arrNodes[startNodeIndex], m_arrNodes[endNodeIndex], m_arrCrSc[EMemberType_FS_Position.BracingBlockPurlins], EMemberType_FS.ePB, EMemberType_FS_Position.BracingBlockPurlins, eccentricityPurlin, eccentricityPurlin, fPBStart_Current, fPBEnd_Current, 0, 0);
                    ...
                    //
                    //    if (j == 0) // First
                    //        fPBStart_Current = (-(float)m_arrCrSc[EMemberType_FS_Position.EdgePurlin].y_max - eccentricityEavePurlin.MFy_local) / (float)Math.Cos(fRoofPitch_rad) - (float)m_arrCrSc[EMemberType_FS_Position.Purlin].z_max * (float)Math.Tan(fRoofPitch_rad) - fCutOffOneSide;
                    //
                    //    for (int k = 0; k < iNumberOfTransverseSupports_Purlins; k++)
                    //    {
                ...
                for (int i = 0; i < vm._canopiesOptionsVM.CanopiesList.Count; i++)
                {
                    ...
                    if (canopyBay.Left)
                    {
                        ...
                        for (int j = 0; j < canopyBay.PurlinCountLeft; j++)
                        {
                            ...
                            m_arrNodes[iCanopy_PurlinNodeIndex + j] = new CNode(iCanopy_PurlinNodeIndex + j + 1, 0 - fCanopy_PurlinCoordinate_x, GetBaysWidthUntilFrameIndex(i), fCanopy_PurlinCoordinate_z, 0);
                        }
                        ...
                        for (int j = 0; j < canopyBay.PurlinCountLeft; j++)
                        {
                            ...
                            m_arrNodes[iCanopy_PurlinNodeIndex + canopyBay.PurlinCountLeft + j] = new CNode(iCanopy_PurlinNodeIndex + canopyBay.PurlinCountLeft + j + 1, 0 - fCanopy_PurlinCoordinate_x, GetBaysWidthUntilFrameIndex(i + 1), fCanopy_PurlinCoordinate_z, 0);
                        }
                    }
                    ...
                    if (canopyBay.Right)
                    {
                        ...
                        for (int j = 0; j < canopyBay.PurlinCountRight; j++)
                        {
                            ...
                            m_arrNodes[iCanopy_PurlinNodeIndex + 2 * canopyBay.PurlinCountLeft + j] = new CNode(iCanopy_PurlinNodeIndex + 2 * canopyBay.PurlinCountLeft + j + 1, fW_frame_centerline + fCanopy_PurlinCoordinate_x, GetBaysWidthUntilFrameIndex(i), fCanopy_PurlinCoordinate_z, 0);
                        }
                        ...
                        for (int j = 0; j < canopyBay.PurlinCountRight; j++)
                        {
                            ...
                            m_arrNodes[iCanopy_PurlinNodeIndex + 2 * canopyBay.PurlinCountLeft + canopyBay.PurlinCountRight + j] = new CNode(iCanopy_PurlinNodeIndex + 2 * canopyBay.PurlinCountLeft + canopyBay.PurlinCountRight + j + 1, fW_frame_centerline + fCanopy_PurlinCoordinate_x, GetBaysWidthUntilFrameIndex(i + 1), fCanopy_PurlinCoordinate_z, 0);
                        }
                    }
            ...
            if (joints == null)
                CreateJoints(bGenerateGirts, bUseMainColumnFlyBracingPlates, bGeneratePurlins, bUseRafterFlyBracingPlates, bGenerateFrontColumns, bGenerateBackColumns, bGenerateFrontGirts,
                             bGenerateBackGirts, bGenerateGirtBracingSideWalls, bGeneratePurlinBracing, bGenerateGirtBracingFrontSide, bGenerateGirtBracingBackSide, bGenerateSideWallCrossBracing, bGenerateRoofCrossBracing, vm._generalOptionsVM.WindPostUnderRafter, LeftColumnGirtNo);
            else
                    ...
                    else if (dp.Validate()) // Ak su vlastnosti dveri validne vyrobime blok dveri a nastavime rebates pre floor slab
                    {
                        ...
                        if (eKitset == EModelType_FS.eKitsetMonoRoofEnclosed)
                        {
                            if(dp.sBuildingSide == "Right")
                                fSideWallHeight = fH2_frame_centerline;
                            else if(dp.sBuildingSide == "Front" || dp.sBuildingSide == "Back")
                                fSideWallHeight = Math.Max(fH1_frame_centerline, fH2_frame_centerline);
                        }
                        AddDoorBlock(dp, LeftColumnGirtNo, RightColumnGirtNo, 0.5f, fSideWallHeight, vm.RecreateJoints);
            ...
            if (vm.WindowBlocksProperties != null)
            {
                foreach (WindowProperties wp in vm.WindowBlocksProperties.ToList())
                {
                    ...
                    else if (wp.Validate())
                    {
                        ...
                        AddWindowBlock(wp, LeftColumnGirtNo, RightColumnGirtNo, 0.5f, fSideWallHeight, vm.RecreateJoints);
                    }
                }
                //refaktoring 24.1.2020
            ...
            AddMembersToMemberGroupsLists(_clVM.ComponentList);
