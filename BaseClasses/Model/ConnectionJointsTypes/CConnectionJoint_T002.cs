...
namespace BaseClasses
{
    public class CConnectionJoint_T002 : CConnectionJointTypes
    {
        ...
        public CConnectionJoint_T002(CNode Node_temp, CMember MainFrameColumn_temp, CMember EavesPurlin_temp, float ft_temp_main_plate, bool bIsDisplayed_temp)
        {
            ...
            CPoint ControlPoint_P1 = new CPoint(0, fControlPointPosition_x, (float)(m_SecondaryMembers[0].CrScStart.y_min - m_fPlate_Angle_Leg), -0.5f * m_SecondaryMembers[0].CrScStart.h - m_ft + flocaleccentricity_z, 0);
            ...
            m_arrPlates[0] = new CConCom_Plate_LL("LLH", ControlPoint_P1, m_fPlate_Angle_Leg, (float)m_SecondaryMembers[0].CrScStart.b, (float)m_SecondaryMembers[0].CrScStart.h, m_fPlate_Angle_Leg, m_ft, 90, 0, 90, iConnectorNumberinOnePlate, fDiameter_temp, fScrewLength, BIsDisplayed); // Rotation angle in degrees
            ...
            if (m_Node.ID != m_SecondaryMembers[0].NodeStart.ID) // If true - joint at start node, if false joint at end node (se we need to rotate joint about z-axis 180 deg)
            {
                ...
                ControlPoint_P1 = new CPoint(0, m_SecondaryMembers[0].FLength - fControlPointPosition_x, (float)(m_SecondaryMembers[0].CrScStart.y_max + m_fPlate_Angle_Leg), -0.5f * m_SecondaryMembers[0].CrScStart.h - m_ft + flocaleccentricity_z, 0);
